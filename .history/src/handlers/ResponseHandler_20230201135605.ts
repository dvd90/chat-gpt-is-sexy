...
import { ERROR_CODES, ICustomRequest, indexedObject } from '../utils';
...
export class ResponseHandler {
  ...
  error(error: Error): express.Response<unknown> {
    ...
    Logger.sentryErr(error, errObj as Record<string, unknown>, this.req?.body);
  ...
  private populateAndPrintMeta(
    ...
    const meta = {
      ...
      company: this.req.company?.name,
      company_id: this.req.company?.company_id,
      method: this.req.method,
      endpoint: this.req.path,
      body: this.req.body,
      query: this.req.query,
